...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate NBE Gated MLP models")
    parser.add_argument("--model_dir", type=str, required=True, help="Directory containing model subdirectories (e.g. outputs/gated_mlp_train)")
    parser.add_argument("--dataset_dir", type=str, required=True, help="Directory containing dataset files")
    parser.add_argument("--output_dir", type=str, default="outputs/gated_mlp_eval", help="Directory to save results")
    ...
    for target_node, dataset in datasets.items():
        ...
        for neighbor_id in node_order:
            if neighbor_id not in physical_outputs_dict:
                # Neighbors must be in physical_outputs_dict (either predicted or GT via force node)
                pass 
...
def run_evaluation(models, node_params, datasets, output_dir):
    ...
    for idx in range(num_samples):
        ...
        for nid, inp in current_inputs.items():
             ...
             model = models[nid]
             # NbeGatedMLP new structure uses embed.weight (d_model, input_dim) -> in_features is at dim 1
             expected_dim = model.embed.in_features
             
             # inp is (1, F)
             if inp.shape[1] != expected_dim:
                 # Check if size mismatch can be fixed by slicing (if fixed node)
                 if inp.shape[1] > expected_dim:
                     # This check is here just in case, similar to simple MLP eval
                     pass
