...
def run_evaluation(models, node_params, neighbor_map, datasets, output_dir):
    ...
    for idx in range(num_samples):
        ...
        # Get time 1 data
        all_next_inputs, all_edge_indices, all_targets = get_nbe_gnn_dataset_time1_data(datasets, idx)
        ...
        # Predict loop
        for t in tqdm(range(target_len - 1), desc=f"Sample {idx+1} steps"):
            all_nbe_outputs = {}
            all_physical_outputs = {}
            
            for node_id, inputs in all_next_inputs.items():
