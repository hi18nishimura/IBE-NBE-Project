...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate NBE Simple Split MLP models")
    parser.add_argument("--model_dir", type=str, required=True, help="Directory containing model subdirectories (e.g. outputs/simple_split_mlp_train)")
    parser.add_argument("--dataset_dir", type=str, required=True, help="Directory containing dataset files")
    parser.add_argument("--output_dir", type=str, default="outputs/simple_split_mlp_eval", help="Directory to save results")
...
def load_models(model_dir, dataset_dir):
    ...
    for subdir in tqdm(subdirs, desc="Loading models"):
        ...
        try:
            ...
            # Default to True for this script as it targets Split MLP
            use_split_mlp = cfg.get('use_split_mlp', True)
        except Exception as e:
    ...
    for target_node, dataset in datasets.items():
        ...
        for neighbor_id in node_order:
            if neighbor_id not in physical_outputs_dict:
                # If physical_outputs_dict is incomplete, skip (should not happen in proper loop)
                pass 
            
            if neighbor_id in physical_outputs_dict:
                phys_val = physical_outputs_dict[neighbor_id].to(device)
                
                # Use target_node's params as approximation (since global_normalize is common)
        ...
        if feature_parts:
            # Concatenate all parts to form single vector
            cat_features = torch.cat(feature_parts, dim=0) 
...
def run_evaluation(models, node_params, datasets, output_dir):
    ...
    for idx in range(num_samples):
        ...
        # Filter inputs if needed (sanity check)
        filtered_inputs = {}
