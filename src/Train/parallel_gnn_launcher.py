"""Launcher to train multiple node-specific GNN models in parallel.

This script spawns multiple processes that call the existing Hydra-backed
trainer `src.Train.nbe_gnn_train`. It limits concurrency and
captures stdout/stderr into per-node launcher logs under the specified
`save_dir/{node_id}` directory.

Usage example:
  python src/Train/parallel_gnn_launcher.py \
    --train_dir /path/to/train --val_dir /path/to/val \
    --node_min 1 --node_max 10 --concurrency 4 --save_dir outputs/gnn \
    --config-name nbe
...
def make_cmd(python: str, train_dir: str, val_dir: str, node_id: int, save_dir: str, config_name: str, overrides: List[str]) -> List[str]:
    ...
    cmd = [python, "-m", "src.Train.nbe_gnn_train"]
...
def parse_args():
    ...
    p.add_argument("--save_dir", default="outputs/gnn_train")
    p.add_argument("--config-name", default="nbe", help="Hydra config name (e.g. nbe)")
