...
from src.Dataloader.nbeDataset import NbeDataset, oka_normalize_dataframe_fast
...
class RecursiveTrainConfig:
    ...
    # We need to know the structure (connections) to build the graph
    node_connection_file: Optional[str] = None
    fixed_nodes_file: Optional[str] = None
    summary_overall_max: Optional[str] = None
    
    # Filter nodes if we only want to train a subset (e.g. for debugging)
    ...
    # model
    load_model_dir: str = MISSING # Where to load pre-trained individual models
    # use_split_mlp: bool = False # Removed as NbeGatedMLP does not have a split variant yet
    ...
    # misc
    save_dir: str = "outputs/recursive_gated_mlp_train"
    seed: int = 42
...
def load_models_and_metadata(cfg: RecursiveTrainConfig, device):
    ...
    for nid in tqdm(target_nodes, desc="Loading Models"):
        ...
        if isinstance(model_cfg, dict) and '_content' in model_cfg:
             model_cfg = model_cfg.get('_content', {})

        state_dict = checkpoint.get('model_state_dict', checkpoint)
        
        # Load architecture params from config, or use defaults
        d_model = model_cfg.get('d_model', 64)
        d_ffn = model_cfg.get('d_ffn', 128)
        seq_len = model_cfg.get('seq_len', 1)
        num_layers = model_cfg.get('num_layers', 6)

        # Infer input/output dims if possible, or try to get from config?
        # Inference from state_dict is tricky with the new architecture.
        # "embed.weight" is (d_model, input_dim)
        # "head.0.weight" is (d_model/2, d_model) -> head structure changed?
        # New head: Linear(d_model, d_model//2) -> GELU -> Linear(d_model//2, output_dim)
        # Last layer: head.2.weight is (output_dim, d_model//2)

        input_dim = None
        output_dim = None

        if 'embed.weight' in state_dict:
            input_dim = state_dict['embed.weight'].shape[1]
        
        if 'head.2.weight' in state_dict:
            output_dim = state_dict['head.2.weight'].shape[0]
        elif 'fc4.weight' in state_dict: # Compatibility for OLD models (if any mixed)
            # This logic supports if we try to load OLD model structure with NEW code (which might fail anyway)
            # But let's assume we are loading NEW models.
            output_dim = state_dict['fc4.weight'].shape[0]

        if input_dim is None:
            print(f"Skipping {nid}: cannot infer input dim (missing embed.weight?)")
            skipped_nodes.append(nid)
            continue
            
        if output_dim is None:
            print(f"Skipping {nid}: cannot infer output dim (missing head.2.weight?)")
            skipped_nodes.append(nid)
            continue
        ...
        try:
            ...
            print(f"Skipping {nid}: failed to load state dict: {e}")
            skipped_nodes.append(nid)
            continue
    ...
    if skipped_nodes:
        print(f"Skipped {len(skipped_nodes)} nodes (best.pth not found or invalid): {skipped_nodes}")
...
def run_training(cfg: RecursiveTrainConfig) -> None:
    ...
    # 5. Loop
    print(f"Starting Recursive Gated MLP Training... Steps: {len(train_loader)} per epoch")
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        for batch_idx, (data, node_ids, force_mask) in enumerate(tqdm(train_loader, desc=f"Epoch {epoch}")):
            ...
            # For BPTT, we must keep graph connected through time.
            # current_state is Variable.
            ...
            for t in range(limit_t - 1):
                ...
                for nid, model in models.items():
                    ...
                    # Direct assignment (slicing)
                    # We need to assign specific columns.
                    # Build mask or indices for features
                    feat_indices = []
                ...
                # Calculate loss
                step_loss = criterion(pred_next_state, gt_next_state)
                batch_loss += step_loss
                
                # Update current_state
                # With teacher forcing chance
                 ...
                 for data, _, force_mask in val_loader:
                    ...
                    for t in range(limit_t - 1):
                        ...
                        for nid, model in models.items():
                            ...
                            n_idx = node_ids_map[nid]
                            is_fixed = fixed_nodes.get(nid, False)
                            feat_indices = [i for i, f in enumerate(features) if not (is_fixed and f in ['dx','dy','dz'])]
                            if feat_indices:
                                pred_next_state[:, n_idx, feat_indices] = out
                        ...
                        batch_val_loss += criterion(pred_next_state, gt_next_state).item()
                        current_state = pred_next_state
...
@hydra.main(version_base=None, config_path="../../config/NBE", config_name="recursive_gated_mlp")
