...
class TrainConfig:
    ...
    # model
    use_split_mlp: bool = False # Option to use SplitMLP
    ...
    # misc
    save_dir: str = "outputs/force_simple_mlp_train"
    seed: int = 42
...
def collate_fn(batch):
    ...
    # inputs in NbeForceDataset already contain force features concatenated
...
def run_training(cfg: TrainConfig) -> None:
    ...
    for node_id in node_ids:
        print(f"Starting training for node_id={node_id} using NbeForceDataset")
        
        train_ds = NbeForceDataset(
        ...
        if _cfg_get('use_split_mlp'):
             model = NbeSimpleSplitMLP(input_dim=input_size, output_dim=output_size)
        else:
             model = NbeSimpleMLP(input_dim=input_size, output_dim=output_size)
...
# Reuse GNN config structure for convenience
@hydra.main(version_base=None, config_path="../../config/NBE", config_name="simple_mlp")
