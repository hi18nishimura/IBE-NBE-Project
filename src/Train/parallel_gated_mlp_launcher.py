"""Launcher to train multiple node-specific Gated MLP models in parallel.

This script spawns multiple processes that call the existing Hydra-backed
trainer `src.Train.nbe_gated_mlp_train`. It limits concurrency and
captures stdout/stderr into per-node launcher logs under the specified
`save_dir/{node_id}` directory.

Usage example:
  python src/Train/parallel_gated_mlp_launcher.py \
    --train_dir /path/to/train --val_dir /path/to/val \
    --node_min 1 --node_max 10 --concurrency 4 --save_dir outputs/gated_mlp_train \
    --config-name gated_mlp
...
def make_cmd(python: str, train_dir: Optional[str], val_dir: Optional[str], node_id: int, save_dir: Optional[str], config_name: str, overrides: List[str]) -> List[str]:
    ...
    cmd = [python, "-m", "src.Train.nbe_gated_mlp_train"]
...
def parse_args():
    ...
    p.add_argument("--config-name", default="gated_mlp", help="Hydra config name (e.g. gated_mlp)")
    ...
    if launcher_save_dir is None:
        try:
            ...
            if config_path.exists():
                ...
                launcher_save_dir = cfg.get("save_dir", "outputs/gated_mlp_train")
                ...
                launcher_save_dir = "outputs/gated_mlp_train"
        except Exception as e:
            print(f"Error reading config: {e}. Using default output path.")
            launcher_save_dir = "outputs/gated_mlp_train"

    node_ids = list(range(args.node_min, args.node_max + 1))
    print(f"Launching Gated MLP training for nodes {args.node_min}..{args.node_max} (total {len(node_ids)}) with concurrency={args.concurrency}")
