...
from src.Evaluate.nbe_gnn_load_weights import load_best_checkpoint, build_model_from_ckpt
...
def clean_omegaconf_dict(cfg: dict) -> dict:
    cleaned_cfg = {}
    for k, v in cfg.items():
        if hasattr(v, '_value'):
            cleaned_cfg[k] = v._value()
        else:
            cleaned_cfg[k] = v
    return cleaned_cfg

def get_model_config(weight_dir: str, node_id: int = 1, model_name: str = "best.pth") -> dict:
    ckpt, _ = load_best_checkpoint(f"{weight_dir}/{node_id}", model_name=model_name)
    if ckpt is None:
        raise FileNotFoundError(f"No checkpoint found in {weight_dir}/{node_id}")
    cfg = ckpt['cfg']
    if isinstance(cfg, dict) and '_content' in cfg:
        cfg = cfg['_content']
    return clean_omegaconf_dict(cfg)
...
# Nbeパラメータの取得
def all_node_nbe_params(node_datasets: dict[int, NbeGNNDataset]) -> dict[int, dict[str, int]]:
    node_params = {}
    all_neighbor_node = {}
...
# NBEの重みを取得する
def all_nbe_weights_load(node_datasets: dict[int, NbeGNNDataset], weight_dir: str, map_location: torch.device | str, model_name: str = "best.pth") -> dict[int, NbeGNN]:
    nbe_models = {}
    for node_id, dataset in tqdm(node_datasets.items(), desc="Weights Load"):
        ckpt, path = load_best_checkpoint(f"{weight_dir}/{node_id}", map_location=map_location, model_name=model_name)
        if ckpt is None:
        ...
        cfg = clean_omegaconf_dict(cfg)
        
        # Get input size from a sample
        sample_item = dataset[0]
        input_size = sample_item["inputs"].shape[-1]
        if cfg.get('force_flag', False):
            input_size += 3
            
        model, _ = build_model_from_ckpt(
            ckpt=ckpt,
            model_ctor=NbeGNN,
            model_ctor_kwargs={
...
def run_single_evaluation(target_idx, all_nbe_dataset, all_nbe_params, all_neighbor_node, all_nbe, device, output_dir, force_flag):
    ...
    if result_df_list:
        ...
        output_feather = output_dir / f"{target_file_stem}_results.feather"
        final_df.to_feather(output_feather)
...
if __name__ == "__main__":
    ...
    # Load config from the first node (assuming consistency)
    print("モデル設定の読み込み")
    # Try to find a valid node directory to load config from
    try:
        cfg = get_model_config(args.weight_dir, node_id=1, model_name=args.model_name)
    except FileNotFoundError:
        # Fallback: try to find any directory in weight_dir
        import os
        subdirs = [d for d in os.listdir(args.weight_dir) if os.path.isdir(os.path.join(args.weight_dir, d))]
        if not subdirs:
             print(f"Error: No subdirectories found in {args.weight_dir}")
             exit(1)
        # Try the first one that looks like an int
        found = False
        for d in subdirs:
            try:
                node_id = int(d)
                cfg = get_model_config(args.weight_dir, node_id=node_id, model_name=args.model_name)
                found = True
                break
            except:
                continue
        if not found:
             print(f"Error: Could not load config from {args.weight_dir}")
             exit(1)

    alpha = cfg.get('alpha', 8.0)
    global_normalize = cfg.get('global_normalize', True)
    force_flag = cfg.get('force_flag', False)
    ...
    # NBEモデルの取得
    print("NBEモデルの取得")
    all_nbe = all_nbe_weights_load(all_nbe_dataset, args.weight_dir, map_location=device, model_name=args.model_name)
    ...
    if args.files_num is not None:
        ...
        target_indices = range(total_files)
        print(f"Processing all {total_files} files by default.")

    for target_idx in tqdm(target_indices, desc="Total Progress"):
        run_single_evaluation(target_idx, all_nbe_dataset, all_nbe_params, all_neighbor_node, all_nbe, device, args.output_dir, force_flag)
